-- ===========================================
-- SCRIPT DE LIMPIEZA V10 - INTEGRIDAD REFERENCIAL COMPLETA
-- FECHA: 2025-09-02
...
UNION ALL
SELECT 'Detalle Ventas totales', COUNT(*) FROM detalle_ventas;
...
-- PASO 7: Eliminar piezas_reparacion_productos asociados a reparaciones antiguas
DELETE FROM piezas_reparacion_productos WHERE reparacion_id IN (SELECT id FROM reparaciones WHERE ticket_id IN (SELECT id FROM tickets WHERE created_at < '2025-08-31'));

-- PASO 8: Eliminar dispositivos asociados a tickets antiguos
DELETE FROM dispositivos WHERE ticket_id IN (SELECT id FROM tickets WHERE created_at < '2025-08-31');

-- PASO 9: Eliminar pagos asociados a tickets antiguos
DELETE FROM pagos WHERE ticket_id IN (SELECT id FROM tickets WHERE created_at < '2025-08-31');

-- PASO 10: Eliminar presupuestos asociados a tickets antiguos
DELETE FROM presupuestos WHERE ticket_id IN (SELECT id FROM tickets WHERE created_at < '2025-08-31');

-- PASO 11: Eliminar reparaciones asociadas a tickets antiguos
...
-- PASO 13: Eliminar detalle_ventas asociados a ventas antiguas
DELETE FROM detalle_ventas WHERE venta_id IN (SELECT id FROM ventas WHERE cliente_id IN (SELECT id FROM clientes WHERE created_at < '2025-08-31'));

-- PASO 14: Eliminar ventas asociadas a clientes antiguos
DELETE FROM ventas WHERE cliente_id IN (SELECT id FROM clientes WHERE created_at < '2025-08-31');

-- PASO 15: Eliminar clientes anteriores al 31/08/2025
...
UNION ALL
SELECT 'Detalle Ventas restantes', COUNT(*) FROM detalle_ventas;
