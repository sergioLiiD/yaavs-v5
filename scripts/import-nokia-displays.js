...
// Datos de displays de Nokia (formato del archivo .txt)
const NOKIA_DISPLAYS = [
  'N0100        G10/G20        ORIG S/M        NEGRO'
];

async function importNokiaDisplays() {
  const result = {
    marca: 'Nokia',
    totalProductos: NOKIA_DISPLAYS.length,
    creados: 0,
    duplicados: 0,
    errores: []
  };
  ...
  try {
    console.log('🚀 Iniciando importación de displays Nokia...\n');
    console.log(`🏷️  Marca: ${result.marca}`);
    console.log(`📦 Productos a importar: ${result.totalProductos}`);

    // Buscar la marca Nokia en la base de datos
    ...
    if (!marca) {
      throw new Error('La marca "Nokia" no existe en la base de datos. Por favor, créala primero.');
    }

    console.log(`✅ Marca encontrada: ${marca.nombre} (ID: ${marca.id})`);

    // Procesar cada producto
    for (const productoLinea of NOKIA_DISPLAYS) {
      try {
        ...
        const nombre = `Display Nokia ${modelo} ${calidad} ${color}`;
    ...
    if (result.creados > 0) {
      console.log(`\n🎉 ¡Importación completada! Se crearon ${result.creados} displays nuevos de Nokia.`);
