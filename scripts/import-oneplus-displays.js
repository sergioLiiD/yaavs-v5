...
// Datos de displays de OnePlus (formato del archivo .txt)
const ONEPLUS_DISPLAYS = [
  'E0100        1+N10        ORIG S/M        NEGRO',
  'E0100+F        1+N10        ORIG C/M        NEGRO',
  'TC100        1+N10        ORIG S/M        NEGRO'
];

async function importOnePlusDisplays() {
  const result = {
    marca: 'OnePlus',
    totalProductos: ONEPLUS_DISPLAYS.length,
    creados: 0,
    duplicados: 0,
    errores: []
  };
  ...
  try {
    console.log('🚀 Iniciando importación de displays OnePlus...\n');
    console.log(`🏷️  Marca: ${result.marca}`);
    console.log(`📦 Productos a importar: ${result.totalProductos}`);

    // Buscar la marca OnePlus en la base de datos
    ...
    if (!marca) {
      throw new Error('La marca "OnePlus" no existe en la base de datos. Por favor, créala primero.');
    }

    console.log(`✅ Marca encontrada: ${marca.nombre} (ID: ${marca.id})`);

    // Procesar cada producto
    for (const productoLinea of ONEPLUS_DISPLAYS) {
      try {
        ...
        const nombre = `Display OnePlus ${modelo} ${calidad} ${color}`;
    ...
    if (result.creados > 0) {
      console.log(`\n🎉 ¡Importación completada! Se crearon ${result.creados} displays nuevos de OnePlus.`);
