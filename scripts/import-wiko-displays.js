...
// Datos de displays de Wiko (formato del archivo .txt)
const WIKO_DISPLAYS = [
  'M0050        T50        ORIG S/M        NEGRO',
  'M0010        T10        ORIG S/M        NEGRO'
];

async function importWikoDisplays() {
  const result = {
    marca: 'Wiko',
    totalProductos: WIKO_DISPLAYS.length,
    creados: 0,
    duplicados: 0,
    errores: []
  };
  ...
  try {
    console.log('🚀 Iniciando importación de displays Wiko...\n');
    console.log(`🏷️  Marca: ${result.marca}`);
    console.log(`📦 Productos a importar: ${result.totalProductos}`);

    // Buscar la marca Wiko en la base de datos
    ...
    if (!marca) {
      throw new Error('La marca "Wiko" no existe en la base de datos. Por favor, créala primero.');
    }

    console.log(`✅ Marca encontrada: ${marca.nombre} (ID: ${marca.id})`);

    // Procesar cada producto
    for (const productoLinea of WIKO_DISPLAYS) {
      try {
        ...
        const nombre = `Display Wiko ${modelo} ${calidad} ${color}`;
    ...
    if (result.creados > 0) {
      console.log(`\n🎉 ¡Importación completada! Se crearon ${result.creados} displays nuevos de Wiko.`);
