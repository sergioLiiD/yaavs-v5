...
// Datos de displays de Alcatel (formato del archivo .txt)
const ALCATEL_DISPLAYS = [
  'AL503        5030 1SE        ORIG S/M        NEGRO',
  'AL524        5024        ORIG S/M        NEGRO',
  'AL528        5028        ORIG S/M        NEGRO'
];

async function importAlcatelDisplays() {
  const result = {
    marca: 'Alcatel',
    totalProductos: ALCATEL_DISPLAYS.length,
    creados: 0,
    duplicados: 0,
    errores: []
  };
  ...
  try {
    console.log('🚀 Iniciando importación de displays Alcatel...\n');
    console.log(`🏷️  Marca: ${result.marca}`);
    console.log(`📦 Productos a importar: ${result.totalProductos}`);

    // Buscar la marca Alcatel en la base de datos
    ...
    if (!marca) {
      throw new Error('La marca "Alcatel" no existe en la base de datos. Por favor, créala primero.');
    }

    console.log(`✅ Marca encontrada: ${marca.nombre} (ID: ${marca.id})`);

    // Procesar cada producto
    for (const productoLinea of ALCATEL_DISPLAYS) {
      try {
        ...
        const nombre = `Display Alcatel ${modelo} ${calidad} ${color}`;
    ...
    if (result.creados > 0) {
      console.log(`\n🎉 ¡Importación completada! Se crearon ${result.creados} displays nuevos de Alcatel.`);
