...
WHERE pr.reparacion_id = (SELECT id FROM reparaciones WHERE ticket_id = 34);
"
...
WHERE prp.reparacion_id = (SELECT id FROM reparaciones WHERE ticket_id = 34);
"
...
echo ""
echo "📋 Salidas de almacén para ticket 34:"
docker exec yaavs_app psql $DATABASE_URL -c "
SELECT 
    id,
    producto_id,
