...
import { pagosIdsEnRangoMX } from '@/lib/reportes/financiero-queries';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pagoIds = await pagosIdsEnRangoMX(fechaInicio, fechaFin);

    const pagos =
      pagoIds.length === 0
        ? []
        : await prisma.pagos.findMany({
            where: { id: { in: pagoIds } },
            include: {
              tickets: {
                include: {
            ...
            orderBy: [{ created_at: 'desc' }, { metodo: 'asc' }],
          });
