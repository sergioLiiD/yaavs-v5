        ...
        <div className="space-y-6">
                  ...
                  {recentTickets.map((ticket) => (
                    <tr key={ticket.id} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusClass(ticket.estado)}`}>
                          {ticket.estado}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <button className="px-3 py-1.5 text-xs text-gray-700 bg-gray-100 hover:bg-gray-200 rounded">
                          Ver
                        </button>
                      </td>
                    </tr>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            <div className="flex items-center mb-3">
              <HiOutlineInformationCircle className="w-5 h-5 text-blue-500 mr-2" />
              <h5 className="text-lg font-medium">Información de sesión</h5>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="text-sm mb-2"><span className="font-semibold">Estado:</span> {status}</p>
              <p className="text-sm mb-2"><span className="font-semibold">Usuario:</span> {session?.user?.name || 'No disponible'}</p>
              <p className="text-sm mb-2"><span className="font-semibold">Email:</span> {session?.user?.email || 'No disponible'}</p>
              <p className="text-sm"><span className="font-semibold">Rol:</span> {session?.user?.role || 'No disponible'}</p>
            </div>
          </div>
        </div>
      </AdminLayout>
